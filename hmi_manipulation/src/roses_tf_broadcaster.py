...
if __name__ == '__main__':
    ...
    roses = []
    roses_left = bed_of_roses("rose_left", NX, NY, BED_X_OFFSET_FROM_CENTER, BED_Y_OFFSET_FROM_CENTER)
    roses += roses_left
    #roses_left = []
    roses_right = bed_of_roses("rose_right", NX, NY, BED_X_OFFSET_FROM_CENTER, -BED_Y_OFFSET_FROM_CENTER - (NY-1)*DISTANCE_BETWEEN_ROSES_Y)
    roses += roses_right

    rospy.set_param("/hmi/roses",roses)

    while not rospy.is_shutdown():
    		#orig: (-0.38, 0, 0.6-0.209)
        
        br.sendTransform(
            (-0.36, 0.0, 0.57-0.209), # laser above ground = 0.209
             tf.transformations.quaternion_from_euler(0, 0, 0),
             rospy.Time.now(),
             "table_top_left",
             "table_reference")
        
        for rose in roses_left:
            #print "rose=", rose
            broadcast_tf(rose[0],rose[1],rose[2], "table_top_left")
        
        br.sendTransform(
            (-0.35, 0.02, 0.57-0.209), # laser above ground = 0.209
             tf.transformations.quaternion_from_euler(0, 0, 0),
             rospy.Time.now(),
             "table_top_right",
             "table_reference")
        
        for rose in roses_right:
            #print "rose=", rose
            broadcast_tf(rose[0],rose[1],rose[2], "table_top_right")
